...
import {
  getHeroPromoProducts,
  getHeroPromoPreviewProducts,
  getHomepageComboHighlights,
...
import { resolveHomepageHeroVariant } from "@/lib/marketing/hero-promo-runtime";
...
interface HomePageProps {
  heroPromoPreview?: boolean;
}

export async function HomePage({ heroPromoPreview = false }: HomePageProps) {
  const shouldUseHeroPromoPreview = heroPromoPreview || process.env.NODE_ENV !== "production";

  const [categoryCards, brandWall, featuredStockProducts, comboHighlights, heroPromo] =
    await Promise.all([
      getHomepageCategories("encargue"),
      getHomepageBrands(),
      getHomepageFeaturedProducts("stock"),
      getHomepageComboHighlights(2),
      Promise.resolve(shouldUseHeroPromoPreview ? getHeroPromoPreviewProducts() : null).then((preview) => preview ?? getHeroPromoProducts()),
    ]);
  ...
  return (
    <div className="relative isolate">
      ...
      {resolveHomepageHeroVariant({
        ...
        topRowCount: heroPromo.topRow.length,
        bottomRowCount: heroPromo.bottomRow.length,
      }) === "promo" ? (
        <HeroPromo topRow={heroPromo.topRow} bottomRow={heroPromo.bottomRow} />
      ) : (
      ...
      {comboHighlights.length > 0 ? (
        <section id="combos-encargue" className="py-14 sm:py-18">
          <Container className="space-y-8">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <SectionHeading
                eyebrow="Combos encargue"
                title="Look completo con descuento real"
                description="Si hay pares activos en encargue, te mostramos top + bottom con el ahorro ya calculado sobre la prenda más barata."
              />

              <Link href="/encargue/combos" className={ghostCtaClassName}>
                Ver look completo
              </Link>
            </div>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {comboHighlights.map((combo) => (
                <article
                  key={combo.comboGroup}
                  className="rounded-[1.85rem] border border-[rgba(210,138,163,0.22)] bg-[linear-gradient(180deg,rgba(255,255,255,0.045),rgba(255,255,255,0.012))] p-6"
                >
                  <p className="text-[11px] tracking-[0.24em] text-[#f1d2dc]/75 uppercase">{combo.comboGroup}</p>
                  <h3 className="mt-3 text-2xl font-semibold text-white">{combo.top.name} + {combo.bottom.name}</h3>
                  <p className="mt-3 text-sm leading-6 text-slate-300">
                    Antes {combo.top.pricing.display} + {combo.bottom.pricing.display}. Ahora total del look: {`$ ${combo.comboPairAmountArs.toLocaleString("es-AR")}`}. 
                    Ahorrás {`$ ${combo.discountAmountArs.toLocaleString("es-AR")}`}.
                  </p>
                  <div className="mt-5 flex flex-wrap gap-3">
                    <Link href={`/${combo.top.availability}/${combo.top.slug}`} className={compactGhostCtaClassName}>
                      Ver top
                    </Link>
                    <Link href={`/${combo.bottom.availability}/${combo.bottom.slug}`} className={compactGhostCtaClassName}>
                      Ver bottom
                    </Link>
                    <Link href="/encargue/combos" className={compactGhostCtaClassName}>
                      Ver look completo
                    </Link>
                  </div>
                </article>
